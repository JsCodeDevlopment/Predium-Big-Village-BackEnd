...
//     }
//   });
  // Rota para criar uma multa para um apartamento.
  fastify.post("/fine/:number", async (req, resp) => {
    try {
      ...
      if (status !== "Canceled" && status !== "PaymentInProgress" && status !== "Paid"){
        resp.status(404).send("Opções de status inválidas. Opções válidas são: Canceled, PaymentInProgress & Paid.")
      }
